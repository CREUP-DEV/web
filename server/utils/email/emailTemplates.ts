...
interface NewsletterDeliveryEmailOptions {
  archiveUrl: string
  coverAlt: string
...
export function buildNewsletterDeliveryEmailHtml(options: NewsletterDeliveryEmailOptions) {
  const coverRow = options.coverUrl
    ? `<tr>
                  <td align="center" style="padding:16px;">
                    <img src="${options.coverUrl}" alt="${options.coverAlt}" width="280"
  ...
  return buildEmailLayout({
    heading: 'Newsletter CREUP',
    preheader: `Ya está disponible la newsletter de CREUP de ${options.monthLabel}`,
    siteUrl: options.siteUrl,
    title: `Newsletter CREUP - ${options.monthLabel}`,
    innerHtml: `<tr>
                  <td align="center" style="padding:24px 16px 8px 16px; font-family:${EMAIL_FONTS.body}; font-size:20px; font-weight:bold; color:${EMAIL_COLORS.body};">
                    ${options.monthLabel}
                  </td>
                </tr>
                ${coverRow}
                <tr>
                  <td style="padding:0 24px 16px 24px; font-family:${EMAIL_FONTS.body}; font-size:16px; line-height:26px; color:${EMAIL_COLORS.body}; text-align:center;">
                    Ya está disponible la newsletter de CREUP del mes de ${options.monthLabel}. Puedes descargarla haciendo clic en el botón de abajo.
                  </td>
                </tr>
                <tr>
                  <td align="center" style="padding:8px 16px 24px 16px;">
                    ${buildEmailButton(options.pdfUrl, 'Descargar Newsletter')}
                  </td>
                </tr>
                ${buildEmailDividerRow()}
                <tr>
                  <td style="padding:16px; font-family:${EMAIL_FONTS.body}; font-size:12px; color:${EMAIL_COLORS.subtle}; line-height:18px; text-align:center;">
                    Recibes este correo porque estás suscrito/a a la newsletter de CREUP.
                    <br />
                    <a href="${options.unsubscribeUrl}" style="color:${EMAIL_COLORS.brand}; text-decoration:none;">Darme de baja</a>
                    · <a href="${options.archiveUrl}" style="color:${EMAIL_COLORS.brand}; text-decoration:none;">Ver todas las newsletters</a>
                  </td>
                </tr>`,
  })
}
