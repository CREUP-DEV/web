...
import { fetchExternalDocumentCollection } from '../external/externalDocumentCollection'
...
export function createPolicyDocumentCollectionRouteHandler(
  ...
  return defineCachedEventHandler(
    async (event: H3Event) => {
      ...
      return {
        data: payload.documents.slice(normalizedOffset, normalizedOffset + normalizedLimit),
        meta: {
